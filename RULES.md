...
1. **NO GALAXY WRITES:** Agents are strictly prohibited from writing or moving files into `private workspace/Knowledge/Galaxy`. All synthesis must land in `02_MACHINE` for human verification.
...
3. **INBOX PRESERVATION:** When processing `private workspace/Inbox`, the original raw note must be archived (e.g., moved to `private workspace/Inbox/Archive`) rather than deleted or overwritten, until the user confirms the new structure is satisfactory.