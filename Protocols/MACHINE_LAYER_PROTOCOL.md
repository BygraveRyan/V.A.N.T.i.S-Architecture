...
Agents are **STRICTLY PROHIBITED** from moving files from `02_MACHINE` directly into `private workspace/Knowledge/Galaxy`. The promotion of intelligence from Machine to Galaxy requires explicit Human Verification.

### 3. File Hygiene
Information in this directory should be considered transient and malleable. Files here are subject to AI overwrites, refactoring, and deletion. If a file is finalized and valuable, it must be moved to an appropriate Human layer (e.g., `private workspace/Projects` or `private workspace/Knowledge`).
