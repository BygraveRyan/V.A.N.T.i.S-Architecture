...
3. > **Memory Layer**: This step writes to **Bucket 1 (Session Memory)** — the per-session recall layer. Future: will also push to Honcho dialect post-sandbox validation.
   APPEND a new ADDENDUM section to the existing `02_MACHINE/State/session-state_YYYY-MM-DD.md`. NEVER overwrite or replace existing content. If the file does not exist yet (first agent of the day), create it fresh using the SESSION_STATE_TEMPLATE. Use this section format:
